...
export const getInvestigation = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/investigation/${payload.id}`, config);

    if (response.status === 200) {
      if (payload.do == 'replace') {
        thunkAPI.dispatch(JSON.parse(JSON.stringify(updateNodes(response.data.data.nodes))))
        thunkAPI.dispatch(updateRelationships(JSON.parse(JSON.stringify(response.data.data.relationships))))
      }

      if (payload.do == 'append') {
        await thunkAPI.dispatch(appendNodes(response.data.data.nodes))
        await thunkAPI.dispatch(appendRelationships(response.data.data.relationships))
      }
    }
  }
);
...
export const getNodeInfo = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/investigation/${payload.id}/address/${payload.uid}`, config);
...
export const getRecentTransactions = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/investigation/${payload.id}/address/${payload.uid}/recent-transactions`, config);
...
export const updateCustomNotes = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    thunkAPI.dispatch(updateCustomNoteInSelectedNode({ custom_note: payload.custom_note, uid: payload.uid }))
    thunkAPI.dispatch(updateCustomNoteInNodes({ custom_note: payload.custom_note, uid: payload.uid }))
    ...
    let response = await axios.patch(`${backendServerBaseURL}/api/investigation/${payload.id}/address/${payload.uid}`, {
...
export const updateNodeColor = createAsyncThunk(
  'investigation/updateCustomNotes',
  async (payload, thunkAPI) => {
    thunkAPI.dispatch(updateColorInSelectedNode({ color: payload.color, uid: payload.uid }))
    thunkAPI.dispatch(updateColorInNodes({ color: payload.color, uid: payload.uid }))
...
export const uploadNetworkImage = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    await payload.canvas.toBlob(async (blob) => {
      ...
      let response = await axios.post(`${backendServerBaseURL}/api/investigation/${payload.id}/image`, data, config);
...
export const investigationSlice = createSlice({
  ...
  reducers: {
    ...
    appendRelationships: (state, action) => {
      ...
      temp_relationships = temp_relationships.concat(final_relationships_to_append)
...
export const selectSelectedNodeInfo = (state) => state.investigation.selectedNodeInfo;
export const selectLoadingNodeInfo = (state) => state.investigation.loadingNodeInfo;
export const selectRecentTransactions = (state) => state.investigation.recentTransactions;
export const selectLoadingRecentTransactions = (state) => state.investigation.loadingRecentTransactions;
