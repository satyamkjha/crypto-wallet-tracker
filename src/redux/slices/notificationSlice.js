...
export const getNotifications = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/notifications`, config);
...
export const getSlackIntegration = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/notifications/slack-integration`, config);

    if (response.status === 200) {
      thunkAPI.dispatch(updateSlackAuthorized(response.data.data.slack_authorized))

      if (response.data.data.slack_authorized == false) {
        thunkAPI.dispatch(updateSlackAuthUrl(response.data.data.auth_url))
      } else {
        thunkAPI.dispatch(updateSlackChannels(response.data.data.slack_channels))
      }
    }
  }
);
...
export const getTelegramIntegration = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    let response = await axios.get(`${backendServerBaseURL}/api/notifications/telegram-integration`, config);

    if (response.status === 200) {
      thunkAPI.dispatch(updateTelegramAuthorized(response.data.data.telegram_authorized))
      thunkAPI.dispatch(updateTelegramNotificationId(response.data.data.telegram_notification_id))
    }
  }
);
...
export const sendSlackCode = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.post(`${backendServerBaseURL}/api/notifications/slack-integration`, { code: payload.code }, config);

    if (response.status === 200 && response.data.message == 'slack authorized successfully') {
...
export const createNotification = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.post(`${backendServerBaseURL}/api/notifications`, {
...
export const updateNotification = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    let response = await axios.patch(`${backendServerBaseURL}/api/notifications`, {
      address: payload.address,
      email: payload.email,
      slackChannelId: payload.slackChannelId,
      telegramChannelEnabled: payload.telegramChannelEnabled ? 1 : 0
...
export const selectOpenCreateNotificationDialog = (state) => state.notification.openCreateNotificationDialog;
export const selectOpenEditNotificationDialog = (state) => state.notification.openEditNotificationDialog;
...
export const selectSlackAuthorized = (state) => state.notification.slackAuthorized;

export const selectTelegramAuthorized = (state) => state.notification.telegramAuthorized;
export const selectTelegramNotificationId = (state) => state.notification.telegramNotificationId;
export const selectopenTelegramNotificationsDialog = (state) => state.notification.openTelegramNotificationsDialog;
