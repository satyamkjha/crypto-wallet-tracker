...
export default function Investigation() {
	...
	const [recentTrnasactionsExpand, setRecentTrnasactionsExpand] =
		useState(false);

	const nodes = useSelector(selectNodes);
	const loadingRecentTransactions = useSelector(
		selectLoadingRecentTransactions
	);
	...
	const processNodes = (nodes) => {
		...
		nodes.map((node) => {
			if (
				VISNodesDataset.current !== null &&
				VISNodesDataset.current.get(node.id) === null
			) {
				nodesArr.push({
					id: node.id,
					uid: node.uid,
	...
	const processEdge = (edges) => {
		...
		edges.map((edge) => {
			if (
				VISEdgesDataset.current !== null &&
				VISEdgesDataset.current.get(edge.id) === null
			) {
					...
					setTimeout(function () {
						if (doubleClick == false) {
							...
							if (nodeids.length > 0) {
								...
								dispatch(
									getNodeInfo({ id: params.id, uid: clickedNodes[0].uid })
								);
								dispatch(
									getRecentTransactions({
										id: params.id,
										uid: clickedNodes[0].uid,
									})
								);
		...
		<>
			...
			<Container
				...
				sx={{
					...
					maxWidth: '1800px',
					width: '95vw',
					display: !loading ? 'bloack' : 'none',
				}}>
				<canvas
					id='viewport'
					style={{ display: 'none', height: 1000, width: 1000 }}></canvas>
				...
				<Box
					sx={{
						...
						alignItems: 'center',
						marginBottom: 3,
						justifyContent: { xs: 'flex-start', md: 'justify-content' },
					}}>
					<Box
						sx={{
							display: 'flex',
									...
									<Grid container>
										...
										<Grid
											...
											<IconButton
												...
												<ContentCopyIcon
													sx={{ color: 'text.secondary', maxWidth: '1rem' }}
												/>
											</IconButton>
										</Grid>
									</Grid>
										...
										return (
											<Box mb={2} mt={1} key={uuidv4()}>
												<Box sx={{ display: 'flex', alignItems: 'center' }}>
													...
													{nodeData.notifications_enabled && (
														<NotificationsIcon
															sx={{ color: 'green', maxWidth: '1rem' }}
														/>
													)}
												</Box>
												<Box
						...
						{selectedEdge == null && (
							<Stack spacing={2}>
													...
													<Grid container>
														<Grid item xs={6}>
															...
															{loadingNodeInfo ? (
																...
																<Typography variant='h6'>
																	{selectedNodeInfo?.first_txn_on
																		? formatDate(selectedNodeInfo?.first_txn_on)
																		: '--/--/----'}
																</Typography>
															)}
														</Grid>
														<Grid item xs={6}>
															...
															{loadingNodeInfo ? (
																...
																<Typography variant='h6'>
																	{selectedNodeInfo?.last_txn_on
																		? formatDate(selectedNodeInfo?.last_txn_on)
																		: '--/--/----'}
																</Typography>
															)}
														</Grid>
												...
												<AccordionSummary expandIcon={<ExpandMoreIcon />}>
													<Typography>
														...
														{selectedNodeInfo?.balances
															? `(${selectedNodeInfo?.balances?.length})`
															: '(0)'}
													</Typography>
												</AccordionSummary>
												<AccordionDetails>
								...
								<Box
									...
									<Accordion
										sx={{ backgroundColor: 'white' }}
										expanded={recentTrnasactionsExpand}>
										<AccordionSummary
											expandIcon={
												<ExpandMoreIcon
													onClick={() => {
														setRecentTrnasactionsExpand(
															!recentTrnasactionsExpand
														);
													}}
												/>
								...
								{!recentTrnasactionsExpand &&
									selectedNode &&
									selectedNode.is_exchange === false && (
										<Box
											p={2}
											sx={{
												border: 'solid',
											...
											<Grid container spacing={2}>
												...
												<Grid item xs={6}>
													<Box
														...
														<Typography variant='caption'>
															{formatDate(new Date(selectedEdge?.timeStamp))}{' '}
															{new Date(
											...
											<Grid container spacing={2}>
												<Grid item xs={6}>
													<Box
														...
														<Typography variant='caption'>
															{selectedEdge?.from_node.address}
														</Typography>
													</Box>
												</Grid>
												<Grid item xs={6}>
